...
export function LocationSearchComponent2({ onLocationSelect }) {
    ...
    const handleSuggestionClick = (location) => {
        ...
        if (nearestCity) {
            ...
            // Close the drawer and redirect to the nearest city's page
            setLocation({
                name: location.name,
            ...
            // Preserve existing query parameters and update the city name
            const currentUrl = new URL(window.location.href);
            const params = currentUrl.searchParams;
            const newUrl = `/${nearestCity.name}${params.toString() ? `?${params.toString()}` : ''}`;
            router.push(newUrl);
        } else {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                try {
                    ...
                    onLocationSelect(fullAddress, latitude, longitude);
    ...
    return (
        <div className="w-[95%] mx-auto mt-4 p-4 border rounded-lg bg-gray-50 shadow-lg">
            <Input
