    ...
    return (
        <div className="flex flex-col  w-full  items-center ">
            <Popover open={open} onOpenChange={setOpen} >
                <PopoverTrigger asChild >
                    <Button
                        ...
                        role="combobox"
                        aria-expanded={open}
                        className="justify-between w-[90%]"
                    >
                        {city ? selectedCity.label : "Select City"}
                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                    </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[200px] z-[1001] p-0">
                    <Command>
            ...
            <Popover open={subCityOpen} onOpenChange={setSubCityOpen}>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        className="justify-between w-[90%] mt-4"
                    >
                        {subCity ? subCity : "Select Area"}
                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                    </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[200px] z-[1001] p-0 max-h-60 overflow-y-auto">
                    <Command>
                                ...
                                {categories.map((categoryObj) => (
                                    ...
                                    >
                                        <Check
                                            className={cn(
                                                "mr-2 h-4 w-4",
                                                category === categoryObj.value ? "opacity-100" : "opacity-0"
                                            )}
                                        />
                                        {categoryObj.label}
                                    </CommandItem>
            ...
            <div className="w-full flex flex-row justify-center mt-3 ">
                <button className="flex flex-row items-center justify-between gap-4 shadow-[inset_0_0_0_2px_#616467] text-black px-12 py-4 rounded-full tracking-widest uppercase font-bold bg-transparent hover:bg-[#616467] hover:text-white dark:text-neutral-200 transition duration-200">
                    Playl   ist <CiSearch className="w-6 h-6" />
                </button></div>
        </div>
    )
}
